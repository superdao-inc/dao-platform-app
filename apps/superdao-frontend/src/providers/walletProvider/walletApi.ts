...
import { MagicLinkApi } from 'src/providers/walletProvider/apis/magicLinkApi';
...
export class WalletApi {
	...
	sendTransaction = async (tx: DefaultTxFragment) => this.api?.sendTransaction(tx);
	...
	private async getConcreteWalletAPI(walletType: string): Promise<BaseWalletApi | undefined> {
		...
		switch (walletType) {
			...
			case 'magiclink': {
				return MagicLinkApi.getInstance();
			}
