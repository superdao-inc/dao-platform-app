...
import { useSendMetaTransactionMutation } from 'src/gql/metaTransaction.generated';
...
export abstract class BaseWalletApi {
	...
	async sendTransaction(transaction: DefaultTxFragment) {
	...
	async sendMetaTransaction(metaTxParams: MetaTxParams): Promise<TransactionResponse | undefined> {
	...
	private mapTransactionToTransactionRequest(transaction: DefaultTxFragment): TransactionRequest {
		const {
			to,
			from,
			nonce,
