...
import {
	MAX_DECIMAL_TAIL_IN_MATIC_PRICE,
	MIN_VISIBLE_TOKEN_AMOUNT,
	MIN_VISIBLE_USD_AMOUNT,
...
export const formatAmountNftString = (maxAmount: number, totalAmount?: number) => {
	if (maxAmount === UNLIMITED_MAX_AMOUNT_VALUE) {
		return 'Unlimited'; // TODO: move 'Unlimited' to translations
	}
	...
	const availableCount = maxAmount - totalAmount;

	if (maxAmount >= 100_000) {
		return availableCount;
	}

	return `${availableCount} of ${maxAmount}`; // TODO: move 'of' to translations
};
