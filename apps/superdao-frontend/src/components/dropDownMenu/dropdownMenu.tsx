...
type Option = Omit<CellProps, 'size' | 'label'> & { label: string };
...
const DropdownMenu = (props: DropdownMenuProps) => {
	...
	return (
				...
				<>
					{options.map((option) => (
						<StyledCell
							...
							size="sm"
							{...option}
							label={<StyledLabel color={option.color}>{option.label}</StyledLabel>}
		...
		>
			{control ? (
				React.cloneElement(control, { ...rest, onClick: handleClick })
			) : (
