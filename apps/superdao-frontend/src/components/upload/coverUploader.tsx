...
export const CoverUploader = (props: Props) => {
	const { label, seed, currentCover, onChange } = props;

	const { t } = useTranslation();

	const [files, uploadWidget] = useUploadWidget({ imagesOnly: true, crop: '16:9' });
	...
	const background = useMemo(() => {
		...
		return currentCover ? `url(${getOptimizedFileUrl(currentCover)})` : generateCoverGradient(seed);
	}, [currentCover, lastUploadedCover, seed]);
