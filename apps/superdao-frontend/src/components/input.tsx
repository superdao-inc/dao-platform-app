...
export const Input = forwardRef<HTMLInputElement, InputProps>((props, ref) => {
	...
	return (
		<Wrapper>
			...
			<InputWrapper isDisabled={disabled} type={type} className={cn(description && 'mt-2')}>
...
const InputWrapper = styled.div<{ isDisabled?: boolean; type?: string }>`
	position: relative;
...
const InputElement = styled.input<{ hasError?: boolean } & Pick<InputProps, 'appearance'>>`
	...
	color: ${({ hasError }) => (hasError ? colors.accentNegativeActive : colors.foregroundPrimary)};
	...
	&::placeholder,
	&:disabled {
		color: ${(props) => (props.hasError ? colors.errorDefault : colors.foregroundSecondary)};
	}

	&:-webkit-autofill,
	&:-webkit-autofill:focus {
		-webkit-box-shadow: inset 0 0 0 1000px ${colors.backgroundTertiary};
		-webkit-text-fill-color: ${({ hasError }) => (hasError ? colors.accentNegativeActive : colors.foregroundPrimary)};
		caret-color: ${colors.foregroundPrimary};
	}
