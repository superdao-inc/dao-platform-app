...
export class ExternalConnector {
	...
	constructor(options: IRequestAmbassadorOptions) {
		...
		this.logger = new Logger(this.connectorName);
	...
	private initRetryListeners() {
		this.retry.onFailure((error) =>
			this.logger.error(`[External Connector retry] failed with options`, {
		...
		this.retry.onRetry((reason) =>
			this.logger.error(`[External Connector retry] retry with options`, {
				reason,
				connectorName: this.connectorName
			})
		);
		...
		this.retry.onGiveUp((reason) =>
			this.logger.error(`[External Connector retry] give up with options`, {
				reason,
				connectorName: this.connectorName
			})
		);
	...
	private initCircuitBreakerListeners() {
		this.circuitBreaker.onFailure((error) =>
			this.logger.error(`[External Connector circuit breaker] failed with options`, {
				handled: error.handled,
				duration: error.duration,
				reason: error.reason,
				connectorName: this.connectorName
		...
		this.circuitBreaker.onBreak((error) =>
			this.logger.error(`[External Connector circuit breaker] break (open) with options`, {
				error,
				connectorName: this.connectorName
			})
		);
